...
import { SIDENAV_ITEMS, SideNavItem, MenuItemWithSubMenuProps} from "@/styles/constants";
...
const sidebar = {
    open: (height = 1000) => ({
        clipPath: `circle(${height * 2 + 100}px at 100% 0)`,
        transition: {
            type: "srping",
            stiffness: 20,
            restDelta: 2,
  ...
  return (
    ...
    >
        ...
        <motion.ul
            ...
            variants={variants}
        >
            {SIDENAV_ITEMS.map((item,index,array) => {
                ...
                return (
                    <div key={index}>
                        {item.subMenu ? (
                            <MenuItemWithSubMenu item={item} toggleOpen={toggleOpen}></MenuItemWithSubMenu>
                        ) : (
                            <MenuItem>
                                <Link 
                                    href={item.path}
...
const MenuToogle = ({toggle}: {toggle: any})=> (
